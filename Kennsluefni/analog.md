...
### Analog inntak
Það þarf **ekki** að nota pinmode() í `void setup(){ }` fyrir **analog** inntaks- eða úttakspinna.
...
1. [Read Analog Voltage with potentiometer (float)](https://www.arduino.cc/en/Tutorial/BuiltInExamples/ReadAnalogVoltage), _prófaðu að nota serial Plotter._
<!--
...
Arduino hefur ekki innbyggðan DAC. Arduino þess í stað vinnur með _pulse-width modulation_ (PWM) bylgju sem er stafrænt merki og er notað til að líkja hliðrænu úttaki. PWM ("analog") pinnar í Arduino (Uno og Nano) eru; 3, 5, 6, 9, 10 og 11. 
Úttaksgildi er á bilinu 0-255. 

![PWM](https://github.com/VESM2VT/Efni/blob/main/Myndir/pwm.gif)

Dæmi um PWM úttak er dimmer og hraðastýring á mótor. Hægt er að nota [map](https://www.arduino.cc/reference/en/language/functions/math/map/) fallið `map(value, fromLow, fromHigh, toLow, toHigh)` frá hliðrænu inntaki (0-1023) yfir í PMW úttak (0-255).<br>
